...
export function TestimonialsSection() {
  ...
  // Store user ratings for each testimonial by id
  const [userRatings, setUserRatings] = useState<{ [id: number]: number }>({})
  ...
  const handleStarClick = (testimonialId: number, rating: number) => {
                    ...
                    <div className="bg-gradient-to-r from-orange-500 to-orange-600 px-6 pt-6 pb-4 relative group-hover:from-orange-600 group-hover:to-orange-700 transition-all duration-500">
                      <div className="flex justify-center mb-4">
                        {[1,2,3,4,5].map((star) => (
                          ...
                          >
                            <Star
                              className={`w-5 h-5 ${star <= rating ? 'text-white fill-current' : 'text-white/50'} transition-colors`}
